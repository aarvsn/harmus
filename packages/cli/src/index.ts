...
program
  .name("harmus")
  .description("Harmus - an autonomous coding agent")
  .version("0.1.0");
...
).action(async (goal: string, opts: { model?: string; provider?: string; models?: string; maxIterations?: number }) => {
  await runCommand({ goal, mode: "plan", cwd: process.cwd(), model: opts.model, provider: opts.provider, modelAssignments: opts.models, maxIterations: opts.maxIterations });
});
...
sharedOptions(
  ...
  await runCommand({ goal, mode: "build", cwd: process.cwd(), model: opts.model, provider: opts.provider, modelAssignments: opts.models, maxIterations: opts.maxIterations });
});
...
).action(async (goal: string, opts: { model?: string; provider?: string; models?: string; maxIterations?: number; yes?: boolean }) => {
  await planThenBuild({ goal, cwd: process.cwd(), model: opts.model, provider: opts.provider, modelAssignments: opts.models, maxIterations: opts.maxIterations, yes: opts.yes });
});
