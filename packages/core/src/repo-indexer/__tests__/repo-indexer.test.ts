...
async function cleanup(dir: string) { await rm(dir, { recursive: true, force: true }); }
...
test("indexRepository on the Harmus core package detects TypeScript", async () => {
  ...
  assert.ok(index.profile.hasTypeScript, `expected hasTypeScript, got profile: ${JSON.stringify(index.profile)}`);
