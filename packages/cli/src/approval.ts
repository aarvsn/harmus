 ...
 */
export async function promptApproval(originalGoal: string, planText: string): Promise<ApprovalResult> {
  ...
  try {
    ...
    while (true) {
      ...
      if (trimmed === "e" || trimmed === "edit") {
        const revised = await rl.question(chalk.bold("Revised goal: "));
